...
export function ZoneHeatmap({
  ...
  if (!mounted) {
    return <div className="w-full h-full min-h-[400px] bg-slate-50 flex items-center justify-center text-slate-400">Loading network map…</div>;
  }
    ...
    <div className="w-full h-full min-h-[500px] relative rounded-xl overflow-hidden border border-slate-200">
      ...
      <div className="absolute bottom-4 left-4 bg-white/95 backdrop-blur shadow-sm border border-slate-200 rounded-lg p-3 z-[400] text-xs">
        <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
          ...
          <div className="flex items-center gap-1.5"><span className="w-4 h-0.5 bg-orange-400 rounded" /><span className="text-slate-600">Moderate</span></div>
          <div className="flex items-center gap-1.5"><span className="w-4 h-0.5 bg-red-500 rounded" style={{ borderTop: '2px dashed #ef4444', background: 'none' }} /><span className="text-slate-600">Burst</span></div>
          <div className="flex items-center gap-1.5"><span className="w-4 h-1 bg-green-500 rounded" /><span className="text-slate-600">Optimal path</span></div>
        </div>
      </div>
    </div>
  );
