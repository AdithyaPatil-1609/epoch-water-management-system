...
function ProposalCard({ prop, onApprove }: { prop: EnrichedProposal; onApprove?: (prop: EnrichedProposal) => void }) {
 ...
 <ArrowRight className="text-slate-900 shrink-0" />
 ...
 <div>
 <div className="text-xs text-slate-800 uppercase tracking-wider mb-0.5">Volume</div>
 <div className="text-lg font-mono text-slate-900">+{prop.volume_ML} <span className="text-sm font-sans text-slate-800">ML/d</span></div>
 </div>
 <div>
 <div className="text-xs text-slate-800 uppercase tracking-wider mb-0.5">Fairness Gain</div>
...
export function ProposalQueue({ proposals, onApprove }: ProposalQueueProps) {
 ...
 return <div className="text-sm text-slate-800 p-4 bg-slate-50 rounded-lg">No active proposals. System is balanced.</div>;
 }
