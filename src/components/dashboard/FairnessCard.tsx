...
export function FairnessCard({ fairness, sourceName, destName, volumeML }: FairnessCardProps) {
 if (!fairness) {
 return (
 <div className="rounded-xl border border-slate-200 bg-slate-50 px-5 py-4 text-sm text-slate-900 flex items-center gap-2">
 <Warning size={18} />
 ...
 const impovColor = improved ? 'text-emerald-600' : noChange ? 'text-slate-800' : 'text-red-500';
 ...
 <span className="text-xs uppercase tracking-widest text-slate-800 mt-1">Fairness Score</span>
 </div>

 {/* Gini before → after */}
 <div className="flex flex-col items-center justify-center text-center border-x border-slate-100 px-3">
 <div className="flex items-center gap-2 text-black font-mono text-sm">
 <span className="text-slate-800">{fairness.gini_before.toFixed(3)}</span>
 <ImprovIcon size={14} className={impovColor} />
 <span className="font-semibold text-slate-900">{fairness.gini_after.toFixed(3)}</span>
 </div>
 <span className="text-xs uppercase tracking-widest text-slate-800 mt-1">Gini Coefficient</span>
 {improved && (
 ...
 <span className="text-base font-sans text-slate-800">%</span>
 </div>
 <span className="text-xs uppercase tracking-widest text-slate-800 mt-1">City Avg Fulfillment</span>
 ...
 <p className="text-xs font-medium text-slate-800 uppercase tracking-wider mb-3">
 ...
 <span className="text-xs font-medium text-black truncate max-w-[140px]" title={z.zone_name}>
 {z.zone_name}
 </span>
 <div className="flex items-center gap-2 ml-2 shrink-0">
 <span className="text-xs font-mono text-slate-900">{z.before_fulfillment_pct}%</span>
 <span className="text-xs text-slate-300">→</span>
 <span className={`text-xs font-mono font-semibold ${z.delta_pct > 0 ? 'text-emerald-700' : z.delta_pct < 0 ? 'text-red-600' : 'text-slate-900'}`}>
...
export function FairnessChip({ fairness }: { fairness: ProposalFairness | null }) {
 ...
 const color = improved ? 'text-emerald-700' : fairness.gini_improvement_pct < 0 ? 'text-red-600' : 'text-slate-800';
