...
function MetricBox({
  ...
  const color = improved ? 'text-emerald-600' : delta === 0 ? 'text-slate-500' : 'text-red-500';

  return (
    <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
      <p className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-2">{label}</p>
      <div className="flex items-baseline gap-2">
        <span className="text-xl font-mono font-semibold text-slate-500 line-through">{format(before)}</span>
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-3 gap-3 mb-3">
          ...
          <div className="bg-slate-50 rounded-lg p-2.5">
            <p className="text-[10px] font-medium text-slate-500 uppercase tracking-wider mb-1">Pressure</p>
            <p className={`text-base font-mono font-bold ${proposal.pressure_impact >= 0 ? 'text-emerald-600' : 'text-orange-600'}`}>
              ...
              <span className="text-xs font-sans text-slate-500"> bar</span>
            </p>
          </div>
          <div className="bg-slate-50 rounded-lg p-2.5">
            <p className="text-[10px] font-medium text-slate-500 uppercase tracking-wider mb-1">Fairness Gain</p>
        ...
        {/* Reason */}
        <p className="text-xs text-slate-600 bg-blue-50 border border-blue-100 rounded-lg p-2.5 mb-3">
          {proposal.reason}
        </p>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-200 disabled:text-slate-400 text-white text-sm font-semibold py-2 px-3 rounded-lg transition-colors"
          >
      ...
      <AnimatePresence>
        {expanded && proposal.fairness && (
          ...
          >
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-2">Zone Fulfillment Impact</p>
                ...
                .map(z => (
                  <div key={z.zone_id}>
                    <div className="flex items-center justify-between text-xs mb-1">
                      <span className="text-slate-700 font-medium truncate max-w-[140px]">{z.zone_name}</span>
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <section className="lg:col-span-5 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col items-center">
            <h2 className="text-sm font-semibold text-slate-700 uppercase tracking-wider self-start mb-4">Network Equality</h2>
